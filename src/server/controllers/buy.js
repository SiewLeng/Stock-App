...
module.exports = db => {
  ...
  let create = (req, res) => {
    ...
    let url = `https://www.alphavantage.co/query?function=CURRENCY_EXCHANGE_RATE&from_currency=${
      company["currency"]
    }&to_currency=SGD&apikey=${apiKey}`;
    ...
    request(url, function(error, queryResponse, body) {
      ...
      let exchangeRate = JSON.parse(body)["Realtime Currency Exchange Rate"][
        "5. Exchange Rate"
      ];
