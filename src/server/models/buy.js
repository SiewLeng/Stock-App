...
module.exports = dbPoolInstance => {
  ...
  const get = callback => {
    let queryString =
      "SELECT * FROM buy INNER JOIN company ON (buy.company_symbol = company.symbol) WHERE buy.buy_id > 0";
  ...
  const create = (symbol, price, priceSGD, numOfStock, company, callback) => {
    ...
    let queryString2 = `INSERT INTO buy (company_symbol, quantity, price, price_sgd)
            VALUES ($1, $2, $3, $4)`;
    let values2 = [symbol, numOfStock, parseFloat(price), parseFloat(priceSGD)];
